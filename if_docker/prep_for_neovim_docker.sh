...
basedir=$(dirname $0)
cp $basedir/Dockerfile $PWD
if [[ "$1" == "-R" ]]; then
  cat $basedir/docker-compose.yml| sed "s/%UID%/0/" | sed "s/%GID%/0/" > $PWD/docker-compose.yml
  ...
  cat $basedir/docker-compose.yml| sed "s/%UID%/$(id -u)/" | sed "s/%GID%/$(id -g)/" > $PWD/docker-compose.yml
fi
