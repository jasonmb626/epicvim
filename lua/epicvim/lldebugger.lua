	...
	["path"] = function(...)
		...
		do
			...
			function Path.getAbsolute(path)
				...
				return Path.format((Path.getCwd() .. Path.separator) .. path)
			end
		end
		return ____exports
	end,
	["sourcemap"] = function(...)
		...
		do
			...
			local function build(data, mapDir, luaScript)
				...
				local sourceMap =
					{ mappings = {}, sources = {}, sourceNames = {}, luaNames = {}, hasMappedNames = false }
				...
				for mapping, separator in mappings:gmatch("([^;,]*)([;,]?)") do
					if #mapping > 0 then
						local colOffset, sourceOffset, sourceLineOffset, sourceColOffset, nameOffset =
							unpack(decodeBase64VLQ(mapping))
						...
						if
							((not lineMapping) or (sourceLine < lineMapping.sourceLine))
							or ((sourceLine == lineMapping.sourceLine) and (sourceColumn < lineMapping.sourceColumn))
						then
							sourceMap.mappings[line] =
			...
			local function findMap(fileName)
				...
				for ____, path in ipairs(getScriptRoots()) do
				...
				if (valType == "table") and not tables[val] then
					...
					if isArray(val) then
						...
						for k, v in pairs(val) do
							...
							table.insert(
								kvps,
								(((("\n" .. indentStr:rep(indent + 1)) .. '"') .. escape(tostring(k))) .. '": ')
									.. valStr
							)
						end
						return (
							(#kvps > 0)
							and (((("{" .. table.concat(kvps, ",")) .. "\n") .. indentStr:rep(indent)) .. "}")
						) or "{}"
					end
		...
		do
			...
			function Send.props(tbl, kind, first, count)
				...
				if kind == "indexed" then
					...
					local lenStatus, tblLen = pcall(function()
						return #tbl
					end)
					...
					elseif tblLen ~= 0 then
						...
						if
							((not mt) and (#dbgProperties.properties == 0))
							or ((mt and luaLenMetamethodSupported) and mt.__len)
						then
							dbgProperties.length = {
								type = type(tblLen),
								value = tostring(tblLen),
			...
			function Send.help(...)
				...
				for _, nameAndDesc in ipairs(helpStrs) do
					...
					table.insert(builtStrs, ((name .. string.rep(" ", (nameLength - #name) + 1)) .. ": ") .. desc)
				end
	...
	["debugger"] = function(...)
		...
		do
			...
			local function debugBreak(activeThread, stackOffset, activeLine)
				...
				local source = Path.format(luaAssert(info.source))
				...
				while true do
					...
					elseif inp:sub(1, 6) == "thread" then
						...
						if newThreadIdStr ~= nil then
							local newThreadId = luaAssert(tonumber(newThreadIdStr))
							local newThread
							for thread, threadId in pairs(threadIds) do
							...
							if newThread ~= nil then
								...
								frame = 0
								frameOffset = ((currentThread == activeThread) and activeThreadFrameOffset)
									or (1 + (threadStackOffsets[currentThread] or 0))
					...
					elseif inp:sub(1, 5) == "frame" then
						...
						if newFrameStr ~= nil then
							local newFrame = luaAssert(tonumber(newFrameStr))
					...
					elseif inp == "locals" then
						local locs =
							getLocals(frame + frameOffset, ((currentThread ~= activeThread) and currentThread) or nil)
					...
					elseif inp == "globals" then
						local globs =
							getGlobals(frame + frameOffset, ((currentThread ~= activeThread) and currentThread) or nil)
					...
					elseif inp:sub(1, 5) == "break" then
						...
						if
							(
								(
									((((cmd == "set") or (cmd == "del")) or (cmd == "delete")) or (cmd == "dis"))
									or (cmd == "disable")
								) or (cmd == "en")
							) or (cmd == "enable")
						then
					...
					elseif inp:sub(1, 5) == "props" then
						...
						elseif ((kind ~= "all") and (kind ~= "named")) and (kind ~= "indexed") then
							Send.error("Bad kind: " .. (("'" .. tostring(kind)) .. "'"))
						else
							local mappedExpression = mapExpressionNames(expression, sourceMap)
							local s, r = execute(
								"return " .. mappedExpression,
				...
				if breakAtDepth >= 0 then
					...
					if stepBreak then
						...
						if
							...
							and (topFrameSource.short_src:sub(1, #builtinFunctionPrefix) == builtinFunctionPrefix)
						then
							return
						end
						local source
				...
				for ____, breakpoint in ipairs(lineBreakpoints) do
					if breakpoint.enabled and comparePaths(breakpoint.file, source) then
						if breakpoint.condition then
							...
							topFrame = topFrame or luaAssert(debug.getinfo(debugHookStackOffset, "nSluf"))
							local success, result = execute(condition, debugHookStackOffset, topFrame)
							if success and result then
								local activeThread = getActiveThread()
								local conditionDisplay = (
									(('"' .. breakpoint.condition) .. '" = "') .. tostring(result)
								) .. '"'
								local breakpointFile, breakpointLine =
									breakpoint.sourceFile or breakpoint.file, breakpoint.sourceLine or breakpoint.line
								Send.debugBreak(
									(
			...
			local function mapSource(indent, file, lineStr, remainder)
				...
				if sourceMap then
					local line = luaAssert(tonumber(lineStr))
					local lineMapping = sourceMap.mappings[line]
					if lineMapping then
						...
						return (
							(
								((((indent .. tostring(sourceFile)) .. ":") .. tostring(sourceLine)) .. ":")
								.. tostring(sourceColumn)
							) .. ":"
						) .. remainder
					end
				end
			...
			local function breakForError(err, level, propagate)
				local message = mapSources(tostring(err))
				level = (level or 1) + 1
			...
			local function useXpcallInCoroutine()
				if canYieldAcrossPcall == nil then
					local _, yieldResult = luaCoroutineResume(luaCoroutineCreate(function()
				...
				if allowBreak and useXpcallInCoroutine() then
					...
					local function debugFunc(...)
						...
						if results[1] then
							...
							skipNextBreak = true
							local message = mapSources(tostring(results[2]))
			...
			function Debugger.pushHook(hookType)
				...
				coroutine.create = function(f)
					return debuggerCoroutineCreate(f, breakInCoroutines)
				end
	...
	["lldebugger"] = function(...)
		...
		function ____exports.runFile(filePath, breakImmediately, arg)
			if type(filePath) ~= "string" then
				error(("expected string as first argument to runFile, but got '" .. type(filePath)) .. "'", 0)
			end
			if (breakImmediately ~= nil) and (type(breakImmediately) ~= "boolean") then
				error(("expected boolean as second argument to runFile, but got '" .. type(breakImmediately)) .. "'", 0)
			end
			local env = setmetatable({ arg = arg }, {
				__index = _G,
		...
		function ____exports.call(func, breakImmediately, ...)
			...
			if type(func) ~= "function" then
				error(("expected string as first argument to debugFile, but got '" .. type(func)) .. "'", 0)
			end
			if (breakImmediately ~= nil) and (type(breakImmediately) ~= "boolean") then
				error(
